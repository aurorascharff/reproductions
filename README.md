...
| [`cases/metadata-suspense-title-timing`](./cases/metadata-suspense-title-timing) | Soft navigation temporarily removes the old static `<title>` before inserting the new one, leaving `document.title` empty; an unrelated layout Suspense boundary does not gate the new title | Authored for friction log |
